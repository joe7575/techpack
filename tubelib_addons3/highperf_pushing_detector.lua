--[[
	...
	highperf_pushing_detector.lua
	
	A high performance pusher that activates when an item is detected
...
local State = tubelib.NodeStates:new({
	node_name_passive = "tubelib_addons3:highperf_pushing_detector",
	node_name_active = "tubelib_addons3:highperf_pushing_detector_active",
	node_name_defect = "tubelib_addons3:highperf_pushing_detector_defect",
	infotext_name = S("HighPerf Pushing Detector"),
...
local function switch_on(pos)
	if tubelib.data_not_corrupted(pos) then
		...
		node.name = "tubelib_addons3:highperf_pushing_detector_active"
...
local function switch_off(pos)
	if tubelib.data_not_corrupted(pos) then
		local meta = M(pos)
		local node = minetest.get_node(pos)
		node.name = "tubelib_addons3:highperf_pushing_detector"
		minetest.swap_node(pos, node)
		State:standby(pos, meta)
	end
end

minetest.register_node("tubelib_addons3:highperf_pushing_detector", {
	description = S("HighPerf Pushing Detector"),
	tiles = {
	...
	after_place_node = function(pos, placer)
		...
		local number = tubelib.add_node(pos, "tubelib_addons3:highperf_pushing_detector")
...
minetest.register_node("tubelib_addons3:highperf_pushing_detector_active", {
	description = S("HighPerf Pushing Detector"),
	tiles = {
		-- up, down, right, left, back, front
		{
	...
	is_ground_content = false,
	sounds = default.node_sound_wood_defaults(),
	drop = "tubelib_addons3:highperf_pushing_detector",
})

minetest.register_node("tubelib_addons3:highperf_pushing_detector_defect", {
	description = S("HighPerf Pushing Detector"),
	tiles = {
		-- up, down, right, left, back, front
		'tubelib_pusher1.png^tubelib_addons3_node_frame4.png',
		...
		'tubelib_addons3_chest_bottom.png^tubelib_defect.png^tubelib_hole.png',
		'tubelib_addons3_chest_bottom.png^tubelib_defect.png^tubelib_hole.png',
	},
...
minetest.register_craft({
	output = "tubelib_addons3:highperf_pushing_detector",
	recipe = {
...
tubelib.register_node("tubelib_addons3:highperf_pushing_detector", 
	{"tubelib_addons3:highperf_pushing_detector_active", "tubelib_addons3:highperf_pushing_detector_defect"}, {
